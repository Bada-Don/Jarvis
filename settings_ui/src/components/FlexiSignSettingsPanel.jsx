...
export default function FlexiSignSettingsPanel({
  ...
  const showProcessName = matchesSearch('Process Name') || matchesSearch('FLEXISIGN_PROCESS_NAME') || matchesSearch(settings.FLEXISIGN_PROCESS_NAME);
  const showExePath = matchesSearch('Executable Path') || matchesSearch('FLEXISIGN_EXE_PATH') || matchesSearch(settings.FLEXISIGN_EXE_PATH);
  const showWindowTitle = matchesSearch('Window Title') || matchesSearch('FLEXISIGN_WINDOW_TITLE') || matchesSearch(settings.FLEXISIGN_WINDOW_TITLE);
  const showModalEnabled = matchesSearch('Startup Modal') || matchesSearch('STARTUP_MODAL_ENABLED');
  const showModalTitle = matchesSearch('Modal Title') || matchesSearch('STARTUP_MODAL_TITLE') || matchesSearch(settings.STARTUP_MODAL_TITLE);
  const showModalButton = matchesSearch('Modal Button') || matchesSearch('STARTUP_MODAL_BUTTON') || matchesSearch(settings.STARTUP_MODAL_BUTTON);
  const showModalTimeout = matchesSearch('Modal Timeout') || matchesSearch('STARTUP_MODAL_TIMEOUT');

  const hasBasicSettings = showProcessName || showExePath || showWindowTitle;
  const hasModalSettings = showModalEnabled || showModalTitle || showModalButton || showModalTimeout;
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-gray-800 mb-2">FlexiSIGN Settings</h2>
        <p className="text-sm text-gray-600">
          Configure settings specific to FlexiSIGN automation for number plate and signage creation.
        </p>
      </div>
          ...
          <div className="space-y-4">
                  ...
                  validation={
                    settings.STARTUP_MODAL_ENABLED
                      ? [
                          {
                            type: 'required',
                            message: 'Modal title is required when modal handling is enabled',
                          },
                        ]
                      : []
                  }
                  ...
                  validation={
                    settings.STARTUP_MODAL_ENABLED
                      ? [
                          {
                            type: 'required',
                            message: 'Button text is required when modal handling is enabled',
                          },
                        ]
                      : []
                  }
            ...
            {!settings.STARTUP_MODAL_ENABLED && !searchQuery && (
              <div className="p-3 bg-blue-50 border border-blue-300 rounded text-sm text-blue-800">
                ℹ️ Startup modal handling is disabled. The system will not automatically handle
                FlexiSIGN's startup dialog.
              </div>
            )}
          </div>
        )}
