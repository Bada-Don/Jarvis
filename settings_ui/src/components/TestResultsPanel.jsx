...
export default function TestResultsPanel({ onClose }) {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'warning':
        return <span className="text-yellow-600 text-base">⚠</span>;
      default:
        return null;
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-gray-800">Test Configuration</h2>
            ...
            <div>
              ...
              <div className="space-y-2">
                {testReport.passed.map((result, index) => (
                  <div
                    key={index}
                    className={`p-4 border rounded-lg ${getStatusColor('passed')}`}
                  >
            ...
            <div>
              ...
              <div className="space-y-2">
                {testReport.failed.map((result, index) => (
                  <div
                    key={index}
                    className={`p-4 border rounded-lg ${getStatusColor('failed')}`}
                  >
                    <div className="flex items-start gap-3">
                      {getStatusIcon('failed')}
                      <div className="flex-1">
                        ...
                        {result.guidance && (
                          <div className="mt-2 p-2 bg-white rounded border border-red-300">
                            ...
                            <div className="text-sm text-gray-700">{result.guidance}</div>
                          </div>
                        )}
                      </div>
                    </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {testReport.warnings.map((result, index) => (
                  <div
                    key={index}
                    className={`p-4 border rounded-lg ${getStatusColor('warning')}`}
                  >
                    <div className="flex items-start gap-3">
                      {getStatusIcon('warning')}
                      <div className="flex-1">
          ...
          <div className="mt-6 p-4 bg-gray-50 border border-gray-200 rounded-lg">
            {testReport.summary.failed_count === 0 ? (
              <div className="flex items-center gap-3 text-green-700">
                ...
                <div>
                  <div className="font-semibold text-lg">Configuration Valid</div>
                  <div className="text-sm">
                    All tests passed successfully. Your configuration is ready to use.
                  </div>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-3 text-red-700">
                <span className="text-xl">✗</span>
                <div>
                  <div className="font-semibold text-lg">Configuration Issues Found</div>
                  <div className="text-sm">
                    Please fix the failed tests above before using this configuration.
                  </div>
                </div>
              </div>
            )}
      ...
      {!testReport && !isLoading && !error && (
        <div className="text-center py-12 text-gray-500">
          ...
          <p className="text-lg">Click "Run Tests" to validate your configuration</p>
