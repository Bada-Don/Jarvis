"""
Firebase Service for JARVIS Backend
Handles Firebase Admin SDK initialization, device registration, messaging, and presence tracking.
"""
...
class FirebaseService:
    """
    Firebase service for backend server.
    Manages device authentication, real-time messaging, and presence tracking.
    """
    ...
    def listen_for_commands(self, device_id: str, callback: Callable[[Dict[str, Any]], None]) -> None:
        """
        Listen for incoming commands for a device.
    ...
    def listen_for_status(self, device_id: str, callback: Callable[[Dict[str, Any]], None]) -> None:
        """
        Listen for status updates from a device.
        
        Args:
            device_id: Device identifier to listen for
            callback: Function to call when status received
        """
        def on_status_added(event):
            """Handle new status event."""
            if event.data is None:
                return
            ...
            print(f"📥 Status received: {message_id}")
            
            # Call callback with status data
            callback(status_data)
        
        # Listen for new status updates
        status_ref = self.db_ref.child('messages').child(device_id).child('status')
        listener = status_ref.listen(on_status_added)
        self._listeners.append(listener)
        
        print(f"👂 Listening for status on device: {device_id}")
    
    def mark_command_processed(self, device_id: str, message_id: str) -> bool:
        """
        ...
        try:
            ...
            print(f"❌ Failed to mark command as processed: {e}")
            return False
